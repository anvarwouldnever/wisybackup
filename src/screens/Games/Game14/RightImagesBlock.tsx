...
const RightImagesBlock = ({ images, offsets, answered, lock, imageLayouts, setIsDrawing, setLines, lineStartX, lineStartY, lineEndX, lineEndY, imageRefs, answersLayouts, answers, answer, setWrongObject, addToAnswered, clicked }) => {
    ...
    return (
        <View style={{width: 'auto', height: 'auto', alignItems: 'center', gap: images?.length === 4 || images?.length === 3 ? s(4) : s(6), justifyContent: 'center', flexDirection: 'column'}}>
            { (images?.length === 4 || images?.length === 5 || images?.length === 6 ? images.slice(3) : []).map((item, index) => {
