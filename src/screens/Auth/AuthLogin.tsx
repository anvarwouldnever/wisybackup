...
const AuthLogin = ({ toggleOption, playersScreen }) => {
    ...
    const signIn = async() => {
        try {
            ...
            if (response.data?.token) {
                store.setChildren(response.data?.children?.data)
    ...
    return (
            <Animated.View entering={FadeInRight} style={{justifyContent: 'space-between', marginTop: 30, alignSelf: 'center', width: width * 0.8666, height: height * 0.7725}}>
                ...
                <View style={{alignItems: 'center', flexDirection: 'column', justifyContent: 'space-between', width: width * 0.8666, height: 'auto', gap: height * (12 / 800)}}>
                    
                    <TextInput 
                        onChangeText={(text) => setEmail(text)} 
                        keyboardType='email-address' 
                        style={{fontWeight: '600', fontSize: height * (14 / 800), padding: height * (16 / 800), borderRadius: 100, borderWidth: 1, borderColor: error? '#D83636' : '#E5E5E5', borderStyle: 'solid', width: width * 0.8666, height: height * (56 / 800)}} 
                    ...
                    {error &&
                        <View style={{width: '100%', alignItems: 'center'}}>
                            <Text style={{width: width * (280 / 360), fontWeight: '600', fontSize: height * (12 / 800), alignItems: 'center', color: '#D83636'}}>{error}</Text>
                        </View>
                    }
                ...
                <TouchableOpacity disabled={!email || !password || password?.length < 8} onPress={() => signIn()} style={{justifyContent: 'center', alignItems: 'center', opacity: email != '' && password != '' && password?.length >= 8? 1 : 0.5, alignSelf: 'center', width: width * 0.8666, height: height * (56 / 800), backgroundColor: '#504297', borderRadius: 100, }}>
